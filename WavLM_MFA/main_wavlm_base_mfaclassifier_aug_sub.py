...
#        database_path, args.seed, config)
    ...
    if args.eval:
        ...
        produce_evaluation_score_file(eval_loader, model, device, eval_score_path)
        print("DONE.")
...
#            output_file=model_tag/"loaded_model_t-DCF_EER.txt")
    ...
    for epoch in range(config["num_epochs"]):
        ...
        if epoch < config['num_epochs'] -4:
            produce_evaluation_file(dev_samp_loader, model, device,
...
#            track, prefix_2019))
    ...
    dev_loader = DataLoader(dev_set,
                            ...
                            drop_last=False,
                            pin_memory=True)
    _, file_dev_samp = genSpoof_list(dir_meta=dev_samp_trial_path,
                                is_train=False,
                                is_eval=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config",
                        ...
                        default="Data3/dh24/asv2024_dh2/config/wavlm_mfaclassifier.conf")
                   #     required=True)
    parser.add_argument(
        "--output_dir",
        dest="output_dir",
