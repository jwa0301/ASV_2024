...
from data_utils import (Dataset_ASVspoof2024_train_aug, Dataset_ASVspoof2024_eval,
                        Dataset_ASVspoof2024_train_aug2,
                        Dataset_ASVspoof2019_devNeval, genSpoof_list)
from evaluation import calculate_tDCF_EER, calculate_EER
...
def main(args: argparse.Namespace,tl,fl) -> None:
    """
    Main function.
...
#    print('LoRA Config : ',config["model_config"]["lora_config"])
    ...
    model_tag = "{}_{}_ep{}_bs{}_lr{}_tl{}_fl{}".format(
        track,
...
#        ,config["model_config"]["lora_config"]["r"])
    ...
    loss_model = OCSoftmax(32, r_real=0.9, r_fake=0.3, alpha=20).to(device) # add
...
#        database_path, args.seed, config)
    ...
    if args.eval:
        ...
        # state_dict = torch.load("Data3/dh24/asv2024_dh/exp_optim_model1/t1_wav2vec2_LCNN_ep10_bs64_lr1e-05_tl9_fl3_experiment3/weights/ocsoftmax_best.pth", map_location=device)
...
#            output_file=model_tag/"loaded_model_t-DCF_EER.txt")
    ...
    ocsoftmax = OCSoftmax(32, r_real=0.9, r_fake=0.3, alpha=20).to(device) # 맨 앞 32는 enc_dim
    ...
    for epoch in range(config["num_epochs"]):
        ...
        if epoch < config['num_epochs'] -4:
            bona_cm, spoof_cm = produce_evaluation_file(dev_samp_loader, model, ocsoftmax, device,  # add bona_cm, spoof_cm
                                metric_path/"dev_score.txt", dev_samp_trial_path)
        else :
            bona_cm, spoof_cm = produce_evaluation_file(dev_loader, model, ocsoftmax, device,       # add bona_cm, spoof_cm
...
#        best_dev_tdcf = min(dev_tdcf, best_dev_tdcf)
        if best_dev_eer >= dev_eer:
            ...
            # loss_model = ocsoftmax # add line
            torch.save(ocsoftmax.state_dict(), 
                      model_save_path / "ocsoftmax_best.pth")  # add line
...
def produce_evaluation_file(
    ...
    for batch_x, utt_id, batch_y in tqdm(data_loader):   # add batch_y
        batch_x = batch_x.to(device)
        batch_y = batch_y.to(device) # add line
        with torch.no_grad():
            feats, batch_out = model(batch_x)  # add line
...
def produce_evaluation_score_file(
    ...
    for batch_x, utt_id in tqdm(data_loader):
        ...
        with torch.no_grad():
            ...
            batch_score =  batch_score.data.cpu().numpy().ravel() # 만약 안되면  batch_score[:, 0]
...
def train_epoch(
    ...
    for batch_x, batch_y in tqdm(trn_loader):
        ...
        batch_loss = batch_ocsoftmaxloss #  batch_ocsoftmaxloss * args.weight_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config",
                        ...
                        default="Data3/dh24/asv2024_dh/config/wav2vec2_LCNN.conf")
                   #     required=True)
    parser.add_argument(
        "--output_dir",
        dest="output_dir",
        ...
        default="Data3/dh24/asv2024_dh/exp_optim_model1",
    )
